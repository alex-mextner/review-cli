...
with the remaining board seats as highest-effort reserve — Fable is excluded from every
preset, including heavy: it's paywalled, review-cli#280). The built-in reviewer board is
...
seats in the selected preset/board (`--preset heavy --pool 0` covers all 9 heavy-preset built-ins;
the raw 10-seat `DEFAULT_BOARD` still includes Fable but no preset dispatches it — review-cli#280).
