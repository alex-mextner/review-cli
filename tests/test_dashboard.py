...
def test_error_recovery_recovered_when_a_clean_call_follows_in_session():
    ...
    with tempfile.TemporaryDirectory() as d:
        ld = Path(d)
        # A panel: a Cloudflare-blocked Qwen seat (403) + a clean DeepSeek seat right after.
        _write_call_log(ld, "20260601T100000_000000", "commandcode", 0, "[stderr] error code: 1010\n",
                        argv0="commandcode API Qwen/Qwen3.7-Max", exit_code=403)
        _write_call_log(ld, "20260601T100002_000000", "commandcode", 0,
                        "## Findings\nA real verdict.\n", argv0="commandcode API deepseek/deepseek-v4-pro", exit_code=0)
        s = p.load_sessions(ld, gap_seconds=90)[0]
        errs = s.errors
        assert len(errs) == 1, errs
        e = errs[0]
        assert e["model"] == "commandcode:Qwen/Qwen3.7-Max", e
...
def test_fallback_resolves_for_real_call_resolved_gateway_ids():
    ...
    # And the specific gateway ids a failing Kimi / GLM call actually carry resolve, not None.
    assert p._fallback_seat_for("commandcode:moonshotai/Kimi-K2.7-Code") is not None
    assert p._fallback_seat_for("zai:glm-5.2") is not None
