...
def test_pass_when_all_assertions_hold():
    auto = _BehaviorFakeAutomation(
        {"commands": {"c": {"notifications": ["Hello there"]}}}
    )
    res = ed.run_ext_suite(
        cases=ed.parse_ext_cases(
            "## Case: hi\nCommand: c\nExpect-notification: Hello\nExpect-no: error\n"
        ),
        automation=auto,
    )
...
def test_fail_on_forbidden_text_across_surfaces():
    auto = _BehaviorFakeAutomation(
        {"commands": {"c": {"notifications": ["Fatal error: boom"]}}}
    )
    res = ed.run_ext_suite(
        cases=ed.parse_ext_cases("## Case: hi\nCommand: c\nExpect-no: error\n"),
        automation=auto,
    )
    assert res.results[0].status == "FAIL"
...
def test_editor_text_assertion():
    auto = _BehaviorFakeAutomation(
        {"commands": {"fmt": {"editor_text": "const formatted = 1;"}}}
    )
    res = ed.run_ext_suite(
        cases=ed.parse_ext_cases(
            "## Case: fmt\nCommand: fmt\nExpect-editor-text: formatted\n"
...
def test_webview_assertion():
    auto = _BehaviorFakeAutomation(
        {"commands": {"panel": {"webview": "<h1>My Panel</h1>"}}}
    )
    res = ed.run_ext_suite(
        cases=ed.parse_ext_cases(
...
def test_action_failure_is_a_fail_not_a_crash():
    ...
    res = ed.run_ext_suite(
        cases=ed.parse_ext_cases("## Case: nope\nCommand: missing.cmd\n"),
        automation=auto,
    )
    assert res.results[0].status == "FAIL"
...
def test_prose_only_case_blocks_the_run():
    auto = _BehaviorFakeAutomation({"commands": {}})
    res = ed.run_ext_suite(
        cases=ed.parse_ext_cases("## Case: prose\njust words, no directives\n"),
        automation=auto,
    )
...
def test_session_continuity_across_cases():
    ...
    auto = _BehaviorFakeAutomation(
        {"commands": {"c": {"notifications": ["Hello world"]}}}
    )
    res = ed.run_ext_suite(
        cases=ed.parse_ext_cases(
            "## Case: act\nCommand: c\n## Case: assert prior\nExpect-notification: Hello\n"
...
def test_blocked_under_strict_still_maps_to_boot_failed_not_strict_flip():
    ...
    verdict, findings, _sev, _cases = parse_qa_results(transcript)
    assert (
        verdict == "BLOCKED" and findings >= 1
    )  # the [P0] bring-up line counts as a finding
    assert (
        verdict_to_exit_code(verdict, findings=findings, strict=True, exit_blocked=8)
        == 8
    )
...
def test_routing_ext_kind_without_config_falls_through():
    ...
    assert (
        _resolve_deterministic_ext(_Ctx(_Args(kind="ext")), _FIXTURES / "web-good")
        is None
    )
...
def test_session_missing_runtime_is_a_controlled_blocked():
    ...
    os.environ["REVIEW_QA_EXT_NODE"] = (
        sys.executable
    )  # a real binary so the runtime resolves
    try:
...
def test_vscode_session_registers_with_the_signal_reaper():
    """review-cli#162 follow-up (codex review): `_VSCodeSession.__enter__` must
    register the runner subprocess with `process._LIVE_CHILDREN` — the same registry
    `install_signal_reaper`'s SIGTERM/SIGINT handler and the internal backstop's
    `kill_live_children()` sweep — so an external signal reaps the whole
    runner+Electron+VS Code tree too, not only `_run_streamed`'s own backend
    children. And it must UNREGISTER once `__exit__` tears the session down."""
    ...
    try:
        ...
        sess.__enter__()
        try:
            assert sess._reaper_handle is not None
            with proc_mod._LIVE_CHILDREN_LOCK:
                live_pids = {p.pid for p, _pgid in proc_mod._LIVE_CHILDREN}
            assert sess._proc.pid in live_pids
        finally:
            proc_pid = sess._proc.pid
            sess.__exit__(None, None, None)
        with proc_mod._LIVE_CHILDREN_LOCK:
            live_pids_after = {p.pid for p, _pgid in proc_mod._LIVE_CHILDREN}
        assert proc_pid not in live_pids_after
    finally:
        restore()


def test_early_exit_before_ready_still_unregisters_from_the_signal_reaper():
    """codex review (review-cli#162 follow-up, PR #164): `__enter__` raising means
    Python NEVER calls `__exit__` (the context-manager protocol) — so a runner that
    exits with an `error` message BEFORE `ready` (or any other `_await_ready`
    failure) used to leave its `_reaper_handle` stuck in `process._LIVE_CHILDREN`
    forever, and any Electron children it may have forked unreaped, since
    `_VSCodeSession._terminate()`'s own cleanup only ran from `__exit__`. Proves the
    fix: after `__enter__` raises `ExtHarnessError`, the process is gone from
    `_LIVE_CHILDREN`."""
    from reviewlib import process as proc_mod
    ...
    try:
        with proc_mod._LIVE_CHILDREN_LOCK:
            pids_before = {p.pid for p, _pgid in proc_mod._LIVE_CHILDREN}
        sess = eh.vscode_session(
            extension_path="/tmp/x", workspace=Path("/tmp"), exit_blocked=8
        )
        try:
            ...
            raise AssertionError(
                "expected ExtHarnessError from an early 'error' runner"
            )
        except eh.ExtHarnessError:
            pass
        # Deliberately do NOT call sess.__exit__() — __enter__ already raised, so a
        # real caller's `with vscode_session(...) as session:` would never reach a
        # matching __exit__ either (the with-statement's __enter__ failed).
        assert (
            sess._proc is None
        )  # _terminate() already ran (from the fix) and cleared it
        assert sess._reaper_handle is None
        with proc_mod._LIVE_CHILDREN_LOCK:
            pids_after = {p.pid for p, _pgid in proc_mod._LIVE_CHILDREN}
        # No new entry survives from this session's runner — the registry is back to
        # exactly what it was before this test spawned anything.
...
def test_env_required_runner_blocks_when_var_missing():
    ...
    try:
        ...
        try:
            ...
            sess.__exit__(None, None, None)
            raise AssertionError(
                "a runner that errors without QA_FLAG must BLOCK when env is empty"
            )
        # present -> BOOTS
...
def test_await_ready_times_out_on_a_silent_but_alive_runner():
    ...
    os.environ["REVIEW_QA_EXT_LAUNCH_TIMEOUT_S"] = (
        "1"  # short deadline so the test is fast
    )
    ...
    try:
        ...
        try:
            ...
            sess.__exit__(None, None, None)
            raise AssertionError(
                "a silent-but-alive runner must BLOCK on the launch timeout"
            )
    finally:
...
def test_read_reply_times_out_on_a_booted_but_silent_runner():
    ...
    try:
        ...
        try:
            ...
            elapsed = _t.monotonic() - start
            assert raised, (
                "a silent-but-alive runner must time the action out, not hang"
            )
            assert elapsed < 10, f"timed out cleanly, not hung (took {elapsed:.1f}s)"
        finally:
            sess.__exit__(None, None, None)
...
def test_read_reply_skips_noise_and_out_of_order_ids():
    ...
    try:
        ...
        auto = sess.__enter__()
        try:
            assert auto.notifications() == [
                "note-A"
            ]  # skipped the noise + the stale id
        finally:
            sess.__exit__(None, None, None)
    finally:
        restore()
...
def test_abs_ext_path_warns_about_escaping_the_worktree():
    ...
    msg = err.getvalue()
    assert (
        "OUTSIDE" in msg and "extension_path" in msg and "not isolated" in msg.lower()
    ), msg
    ...
    with contextlib.redirect_stderr(err_t):
        _qa_mod._warn_abs_path_escapes_worktree(
            Path("/tmp/review-qa-wt-x"), False, trav_cfg
        )
...
def test_live_vscode_dod_good_passes():
    ...
    if not _vscode_ready():
        _skip(
            "REVIEW_QA_VSCODE=1 + node/tsx + a VS Code binary (set VSCODE_PATH or `code` on PATH)"
        )
        return
